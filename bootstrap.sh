...
if [[ "$adguard_enable" =~ ^[yY]$ ]]; then
  ...
  $SUDO .venv/bin/certbot certonly --non-interactive --break-my-certs --force-renewal --agree-tos --email root@localhost.com --standalone --staging -d $root_host -d vpn.$root_host -d auth.$root_host || $SUDO .venv/bin/certbot certonly --non-interactive --force-renewal --agree-tos --email root@localhost.com --standalone -d $root_host -d vpn.$root_host -d auth.$root_host  || exit
fi
echo "OK"
